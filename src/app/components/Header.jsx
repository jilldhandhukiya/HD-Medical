...
// Static navigation links - moved outside component
const NAV_LINKS = [
  { title: 'Home', url: '/' },
...
export default function Header() {
  ...
  const headerClasses = useMemo(() => 
    `w-full py-6 px-6 md:px-12 flex justify-between items-center fixed top-0 left-0 z-50 transition-all duration-300 max-w-full ${
      scrolled ? 'bg-white shadow-md' : 'bg-transparent'
    } ${isOpen ? 'bg-white' : ''} ${headerVisible ? 'translate-y-0' : '-translate-y-full'}`,
    [scrolled, isOpen, headerVisible]
  );
